...
f=this.startRepeat?this.startRepeat.parentNode:this.callbackElement,g=this.proxies,h=Q.cloneNode(!1);if("del"===b||"move"===b)var k=Fa(f,this,c);switch(b){case "add":for(var l=d,m=this.getter().length-1,e=[],n={},k=0,p=l.length;k<p;k++){var n=k+c,r=pc(n,l[k],this,m);g.splice(n,0,r);n=vb(this,h,e,r)}k=Fa(f,this,c);n.node=k;n.parent=f;f.insertBefore(h,k);for(k=0;n=e[k++];)pa(n,this.vmodels);break;case "del":h=g.splice(c,d);va(h);Ea(wb(k,e,d));break;case "index":for(m=g.length-1;d=g[c];c++)d.$index=
...
if(3===d.nodeType){d.element=e.parentNode;try{e.data=b}catch(f){}}else"textContent"in c?c.textContent=b:c.innerText=b},visible:function(b,c,d){c.style.display=b?d.display:"none"},widget:A},Ib=/\(([^)]*)\)/,Zc=/^\s+$/;avalon.parseDisplay=ca;var E=avalon.bindingHandlers={attr:function(b,c){var d=b.value.trim(),e=!0;-1<d.indexOf(L)&&2<d.indexOf(M)&&(e=!1,ba.test(d)&&(""===RegExp.rightContext&&""===RegExp.leftContext)&&(e=!0,d=RegExp.$1));b.handlerName="attr";N(d,c,b,e?null:ya(b.value))},"class":function(b,
c){var d=b.param,e=b.value;b.handlerName="class";if(!d||isFinite(d)){b.param="";var f=e.replace(Db,function(b){return Math.pow(10,b.length-1)}).indexOf(":");if(-1===f)d=e;else if(d=e.slice(0,f),e=e.slice(f+1),qa(e,c,b),b.evaluator)b._evaluator=b.evaluator,b._args=b.args;else return B("debug: ms-class '"+(e||"").trim()+"' \u4e0d\u5b58\u5728\u4e8eVM\u4e2d"),!1;e=ba.test(d);e||(b.immobileClass=d);N("",c,b,e?ya(d):null)}else b.immobileClass=b.oldStyle=b.param,N(e,c,b)},duplex:function(b,c){var d=b.element;
...
91===c||(15<c&&19>c||37<=c&&40>=c)||("cut"===b.type?avalon.nextTick(r):r())};ka.forEach(function(c){b.attachEvent("on"+c,la)});d.rollback=function(){ka.forEach(function(c){b.detachEvent("on"+c,la)})}}b.oldValue=b.value;Jb(function(){if(avalon.contains(z,b))fc.call(b);else if(!b.msRetain)return!1});ua(d);var Qb=setTimeout(function(){k||m.call(b,b.value);clearTimeout(Qb)},31)};var rb,da=[],Jb=A;avalon.tick=function(b){1===da.push(b)&&(rb=setInterval(gc,30))};try{var Ta=HTMLInputElement.prototype;Object.getOwnPropertyNames(Ta);
