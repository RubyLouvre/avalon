...
t&&V(e.get)&&2>=Object.keys(e).length){var H=e.set,F=e.get;k=function(b){var e=f.$events,g=d[c];if(arguments.length){if(ja)return;if(V(H)){var h=e[c];e[c]=[];H.call(f,b);e[c]=h}}else avalon.openComputedCollect&&ka(e[c]);b=d[c]=F.call(f);if(!la(g,b)){if(ma&&(h=W[f.$id])&&h[c])h[c].$val=b;va(e[c]);f.$events&&R.$fire.call(f,c,b,g)}return b};l.push(function(){Y[A]={evaluator:k,element:J,type:"computed::"+c,handler:y,args:[]};k();ka(g[c]);delete Y[A]})}else wa.test(t)?(k=function(b){var e=k.child,h=d[c];
if(arguments.length)!ja&&!la(h,b)&&(e=k.child=Yb(f,c,b,t),b=d[c]=e.$model,(e=xa[e.$id])&&e(),f.$events&&R.$fire.call(f,c,b,h));else return ka(g[c]),e},(k.child=N(e,0,d[c])).$events[G]=g[c]):k=function(b){var e=d[c];if(arguments.length){if(!la(e,b)){d[c]=b;if(ma){var h=W[f.$id];h&&h[c]&&(h[c].$val=b)}va(g[c]);f.$events&&R.$fire.call(f,c,b,e)}}else return ka(g[c]),e};h[c]=k}})(e,b[e]);ta.forEach(function(c){delete b[c];delete d[c]});var f=na(f,Zb(h),b),k;for(k in b)h[k]||(f[k]=b[k]);f.$id=ha();f.$model=
...
$first:0===b,$last:b===e};b[f]=c;e=0;for(var h=da.length;e<h;e++)if(g=da[e],g.hasOwnProperty(f)&&avalon.type(g[f])===avalon.type(c)){for(var l in b)g[l]=b[l];da.splice(e,1);g.$watch(f,function(b){d.$repeat.set(g.$index,b)});return g}wa.test(avalon.type(c))&&(b.$skipArray=[f]);g=N(b,zc);g.$watch(f,function(b){d.$repeat.set(g.$index,b)});g.$id=("$proxy$"+d.type+Math.random()).replace(/0\./,"");return g}function Eb(b){for(var c=0,d;d=b[c++];){var e=void 0;for(e in d.$events)Array.isArray(d.$events[e])&&
(d.$events[e].length=0);da.unshift(d)>u.maxRepeatSize&&da.pop()}b.length=0}function T(){p.body&&(D?(s["ready!"].state=2,D.checkDeps()):Fb.forEach(function(b){b(avalon)}),T=y)}function Gb(){try{B.doScroll("left"),T()}catch(b){setTimeout(Gb)}}var A=new Date-0,G="$"+A,r=Function("return this")(),Ac=r.require,Bc=r.define,ja=!1,z=/[^, ]+/g,Hb=/\[native code\]/,wa=/^(?:object|array)$/,Ib=/^\[object SVG\w*Element\]$/,Vb=/^\[object (?:Window|DOMWindow|global)\]$/,Ia=Object.prototype,O=Ia.hasOwnProperty,ia=
...
(h=this.sortedCallback.call(e,m))&&(Array.isArray(h)&&h.length)&&(m=h);for(h=0;k=m[h++];)"hasOwnProperty"!==k&&Bb(this,g,f[k],l);this.proxySize=m.length;e.insertBefore(g,this.element.nextSibling);for(h=0;g=l[h++];)Ba(g.nodes,g.vmodels),g.nodes=g.vmodels=null;Db(this)}var H=this.renderedCallback||y,F=arguments;qa(e,function(){H.apply(e,F);e.oldValue&&("SELECT"===e.tagName&&"index"===b)&&avalon(e).val(e.oldValue.split(","))},NaN)}},html:function(b,c,d){b=null==b?"":b;var e="group"in d?c.parentNode:
...
!1;e=aa.test(d);e||(b.immobileClass=d);K("",c,b,e?Ca(d):0)}else b.immobileClass=b.oldStyle=b.param,K(e,c,b)},duplex:function(b,c){var d=b.element,e;K(b.value,c,b,0,1);b.changed=fa(d,"data-duplex-changed",c)||y;if(b.evaluator&&b.args){var f=[],g=M("string,number,boolean,checked");"radio"===d.type&&""===b.param&&(b.param="checked");d.msData&&(d.msData["ms-duplex"]=b.value);b.param.replace(/\w+/g,function(c){/^(checkbox|radio)$/.test(d.type)&&/^(radio|checked)$/.test(c)&&("radio"===c&&v("ms-duplex-radio\u5df2\u7ecf\u66f4\u540d\u4e3ams-duplex-checked"),
...
var t=function(b){setTimeout(function(){h(b)})};l.replace(z,function(b){switch(b){case "input":w?(k("input",h),p.documentMode||(k("compositionstart",f),k("compositionend",g)),9===p.documentMode&&(k("paste",t),k("cut",t),k("keydown",function(b){8===b.keyCode&&t()}))):k("propertychange",function(b){"value"===b.propertyName&&h()});break;default:k(b,h)}})}b.oldValue=b.value;Tb(function(){if(avalon.contains(B,b))yb.call(b);else if(!b.msRetain)return!1});ya(d);e.call(b,b.value)};var zb,ca=[],Tb=y;avalon.tick=
function(b){1===ca.push(b)&&(zb=setInterval(tc,60))};try{var Va=HTMLInputElement.prototype;Object.getOwnPropertyNames(Va);var vc=Object.getOwnPropertyDescriptor(Va,"value").set;Object.defineProperty(Va,"value",{set:uc})}catch(qd){Tb=avalon.tick}X.SELECT=function(b,c,d){var e=avalon(b);d.handler=function(){var d=c(),d=d&&d.$model||d;Array.isArray(d)?b.multiple||v("ms-duplex\u5728<select multiple=true>\u4e0a\u8981\u6c42\u5bf9\u5e94\u4e00\u4e2a\u6570\u7ec4"):b.multiple&&v("ms-duplex\u5728<select multiple=false>\u4e0d\u80fd\u5bf9\u5e94\u4e00\u4e2a\u6570\u7ec4");
d=Array.isArray(d)?d.map(String):d+"";d+""!==b.oldValue&&(e.val(d),b.oldValue=d+"")};d.bound("change",function(){if(!1!==e.data("duplex-observe")){var f=e.val(),f=Array.isArray(f)?f.map(function(b){return d.pipe(b,d,"get")}):d.pipe(f,d,"get");f+""!==b.oldValue&&c(f);d.changed.call(b,f,d)}});qa(b,function(){ya(d);d.changed.call(b,c(),d)},NaN)};X.TEXTAREA=X.INPUT;var ga=avalon.eventHooks;"onmouseenter"in B||avalon.each({mouseenter:"mouseover",mouseleave:"mouseout"},function(b,c){ga[b]={type:c,deel:function(c,
...
c?"floor":"ceil"](c/60),2)+b(Math.abs(c%60),2))}},g=/((?:[^yMdHhmsaZE']+)|(?:'(?:[^']|'')*')|(?:E+|y+|M+|d+|H+|h+|m+|s+|a|Z))(.*)/,h=/^\d+$/,l=/^(\d{4})-?(\d+)-?(\d+)(?:T(\d+)(?::?(\d+)(?::?(\d+)(?:\.(\d+))?)?)?(Z|([+-])(\d+):?(\d+))?)?$/,k=/^(\d+)\D(\d+)\D(\d+)/;Xa.date=function(b,c){var d=Xa.date.locate,l="",F=[],ua,m;c=c||"mediumDate";c=d[c]||c;"string"===typeof b&&(h.test(b)?b=parseInt(b,10):(b=b.trim().replace(k,function(b,c,d,e){return(4===e.length?[e,c,d]:[c,d,e]).join("-")}),b=e(b)),b=new Date(b));
"number"===typeof b&&(b=new Date(b));if("date"===avalon.type(b)){for(;c;)(m=g.exec(c))?(F=F.concat(m.slice(1)),c=F.pop()):(F.push(c),c=null);F.forEach(function(c){ua=f[c];l+=ua?ua(b,d):c.replace(/(^'|'$)/g,"").replace(/''/g,"'")});return l}};var m={AMPMS:{"0":"\u4e0a\u5348",1:"\u4e0b\u5348"},DAY:{"0":"\u661f\u671f\u65e5",1:"\u661f\u671f\u4e00",2:"\u661f\u671f\u4e8c",3:"\u661f\u671f\u4e09",4:"\u661f\u671f\u56db",5:"\u661f\u671f\u4e94",6:"\u661f\u671f\u516d"},MONTH:{"0":"1\u6708",1:"2\u6708",2:"3\u6708",
