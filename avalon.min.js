...
"el",g;b={$remove:function(){return d.$repeat.removeAt(g.$index)},$itemName:f,$index:b,$outer:d.$outer,$first:0===b,$last:b===e};b[f]=c;e=0;for(var h=da.length;e<h;e++)if(g=da[e],g.hasOwnProperty(f)&&avalon.type(g[f])===avalon.type(c)){for(var k in b)g[k]=b[k];da.splice(e,1);return g}wa.test(avalon.type(c))&&(b.$skipArray=[f]);g=N(b,zc);g.$watch(f,function(b){d.$repeat.set(g.$index,b)});g.$id=("$proxy$"+d.type+Math.random()).replace(/0\./,"");return g}function Eb(b){for(var c=0,d;d=b[c++];){var e=
void 0;for(e in d.$events)Array.isArray(d.$events[e])&&(d.$events[e].length=0);da.unshift(d)>u.maxRepeatSize&&da.pop()}b.length=0}function T(){p.body&&(D?(s["ready!"].state=2,D.checkDeps()):Fb.forEach(function(b){b(avalon)}),T=y)}function Gb(){try{B.doScroll("left"),T()}catch(b){setTimeout(Gb)}}var A=new Date-0,G="$"+A,r=Function("return this")(),Ac=r.require,Bc=r.define,ja=!1,z=/[^, ]+/g,Hb=/\[native code\]/,wa=/^(?:object|array)$/,Ib=/^\[object SVG\w*Element\]$/,Vb=/^\[object (?:Window|DOMWindow|global)\]$/,
