...
<h1>Avalon</h1>
<p>迷你简单易用的MVVM框架</p>
<p>前端做久了才会发现HTML(DOM)才是所有前端的真正精髓汇聚地。不管JS和CSS都是为DOM服务的。但是DOM遍布荆棘与陷阱，因此才出现像jQuery那样专门为它填坑的库，
前端开发就是一边填坑一边写业务中进行。avalon的出现，将改变这破局，脱离DOM的藩蓠，专注于需求本身，将可变的数据与操作数据的方法封装成模型。
