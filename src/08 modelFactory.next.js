...
var makeComputedAccessor = function (name, options) {
    ...
    function accessor(value) {//计算属性
        ...
        if (arguments.length > 0) {
            ...
            if (initializing || haveDependenciesChanged(dependencies)) {
                oldValue = evaluateImmediate(this, false)
                updateDependencies(dependencies)
            }
            return oldValue
        }
    ...
    function evaluateImmediate(vmodel, isNotify) {
        var oldValue = accessor._value
        dependencyDetection.begin({
            callback: function (dependency) {//dependency为一个accessor
                ...
                if (!dependencies[name]) {
                    ...
                    dependency.collect(vmodel, evaluateImmediate)
                }
            }
        })
        try {
...
function accessorFactory(accessor, name) {
    ...
    accessor.collect = function (vmodel, evaluate) {
        var array = vmodel.$events[this._name]
        if (array) {
            array.push(function () {
                return evaluate(vmodel, true)
            })
        }
    }
...
var dependencyDetection = (function () {
    ...
    var currentFrame
    return {
        begin: function (accessor) {
            outerFrames.push(currentFrame)
            currentFrame = accessor
        },
        end: function () {
            currentFrame = outerFrames.pop()
        },
        registerDependency: function (accessor) {
            if (currentFrame) {
                //被dependencyDetection.begin调用
                currentFrame.callback(accessor);
            }
        }
    };
})()
